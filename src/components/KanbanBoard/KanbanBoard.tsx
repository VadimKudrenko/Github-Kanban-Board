  ...
  useEffect(()=>{
    ...
    if (repositoryData.html_url !== undefined && sessionStorage.getItem(repositoryData.html_url) !== null)  {
      ...
      if(currentRepoSessionStorageToParse !== null) {
        console.log(currentRepoSessionStorageToParse.replaceAll('"', ''))
      ...
      const doneIssuesBoard = repositoryIssues.filter(issue => issue.closed_at !== null)
      setBoards([
        {id: 1, title: 'To Do', issues: toDoIssuesBoard},
        {id: 2, title: 'In Progress', issues: inProgressIssuesBoard},
        {id: 3, title: 'Done', issues: doneIssuesBoard}
      ])
    } else {
      if(currentRepoSessionStorageToParse !== null) {
        const currentRepoSessionStorage = JSON.parse(currentRepoSessionStorageToParse);
